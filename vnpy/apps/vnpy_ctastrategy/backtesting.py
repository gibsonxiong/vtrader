...
from .template import CtaTemplate, PositionMgr
...
class BacktestingEngine:
    ...
    def __init__(self) -> None:
        ...
        self.result_id: int = None  # 回测结果ID
    ...
    def calculate_result(self) -> DataFrame:
        ...
        for trade in self.trades.values():
            d: Date = trade.trade_time.date()
            daily_result: DailyResult = self.daily_results[d]
            daily_result.add_trade(trade)

        prev_daily_result = None
        # Calculate daily result by iteration.
        for daily_result in self.daily_results.values():
            daily_result.calculate_pnl(prev_daily_result)
            prev_daily_result = daily_result
    ...
    def update_daily_close(self, price: float) -> None:
        ...
        if not daily_result:
            daily_result = DailyResult(d, price)
            self.daily_results[d] = daily_result
            
        daily_result.close_price = price
        posMgr = self.strategy.posMgr
        daily_result.long_pos = posMgr.long_pos
        daily_result.short_pos = posMgr.short_pos
        daily_result.long_price = posMgr.long_price
        daily_result.short_price = posMgr.short_price
        daily_result.long_realized_pnl = posMgr.long_realized_pnl
        daily_result.short_realized_pnl = posMgr.short_realized_pnl
        daily_result.long_pnl = posMgr.get_long_pnl(price)
        daily_result.short_pnl = posMgr.get_short_pnl(price)
        daily_result.long_win_count = posMgr.long_win_count
        daily_result.short_win_count = posMgr.short_win_count
        daily_result.long_loss_count = posMgr.long_loss_count
        daily_result.short_loss_count = posMgr.short_loss_count
        daily_result.long_win_rate = posMgr.long_win_rate
        daily_result.short_win_rate = posMgr.short_win_rate
    ...
    def cross_limit_order(self) -> None:
        ...
        for order in list(self.active_limit_orders.values()):
            ...
            long_cross: bool = (
                 (
                    (order.direction == Direction.LONG and order.offset == Offset.OPEN) or
            ...
            short_cross: bool = (
                (
                    (order.direction == Direction.SHORT and order.offset == Offset.OPEN) or
            ...
            trade: TradeData = TradeData(
                ...
                commission=trade_price * order.volume * self.rate,
            )

            self.strategy.posMgr.update_pos(trade, True)
...
class DailyResult:
    ...
    def __init__(self, date: Date, close_price: float) -> None:
        ...
        self.daily_long_realized_pnl: float = 0
        self.daily_short_realized_pnl: float = 0
        self.long_pnl: float = 0
        self.short_pnl: float = 0
        self.daily_long_pnl: float = 0
        self.daily_short_pnl: float = 0
    ...
    def calculate_pnl(
        self,
        prev_result: Optional["DailyResult"] = None,
    ) -> None:
        """"""
        ...
        # 计算手续费
        self.commission = sum([trade.commission for trade in self.trades])
        self.daily_long_realized_pnl = self.long_realized_pnl
        self.daily_short_realized_pnl = self.short_realized_pnl
        self.daily_long_pnl = self.long_pnl
        self.daily_short_pnl = self.short_pnl

        if (prev_result is not None):
            self.daily_long_realized_pnl = self.long_realized_pnl - prev_result.long_realized_pnl
            self.daily_short_realized_pnl = self.short_realized_pnl - prev_result.short_realized_pnl

            self.daily_long_pnl = self.long_pnl - prev_result.long_pnl
            self.daily_short_pnl = self.short_pnl - prev_result.short_pnl

            long_win_count = self.long_win_count - prev_result.long_win_count
            short_win_count = self.short_win_count - prev_result.short_win_count
            long_loss_count = self.long_loss_count - prev_result.long_loss_count
            short_loss_count = self.short_loss_count - prev_result.short_loss_count
            long_wl_count = long_win_count + long_loss_count
            short_wl_count = short_win_count + short_loss_count
            self.long_win_rate = long_win_count / long_wl_count if long_wl_count > 0 else 0
            self.short_win_rate = short_win_count / short_wl_count if short_wl_count > 0 else 0

        self.daily_realized_pnl = self.daily_long_realized_pnl + self.daily_short_realized_pnl
        self.daily_pnl = self.daily_long_pnl + self.daily_short_pnl
        self.total_pnl = self.daily_realized_pnl + self.daily_pnl
