...
class DownloadDialog(QtWidgets.QDialog):
    ...
    def __init__(self, engine: ManagerEngine, parent=None) -> None:
        ...
        self.symbol_edit: QtWidgets.QLineEdit = QtWidgets.QLineEdit("BTCUSDT_SWAP")
        ...
        end_dt: datetime = datetime.now().replace(tzinfo=DB_TZ)
    ...
    def download(self) -> None:
        ...
        end_date_edit = self.end_date_edit.date()
        start: datetime = datetime(start_date.year(), start_date.month(), start_date.day())
        end: datetime = datetime(end_date_edit.year(), end_date_edit.month(), end_date_edit.day())
        start: datetime = start.replace(tzinfo=DB_TZ)
        end: datetime = end.replace(tzinfo=DB_TZ)

        if interval == Interval.TICK:
            count: int = self.engine.download_tick_data(symbol, exchange, start, end, self.output)
        else:
            count: int = self.engine.download_bar_data(symbol, exchange, interval, start, end, self.output)
