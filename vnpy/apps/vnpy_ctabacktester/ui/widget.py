...
from vnpy.trader.ui.widget import BaseMonitor, BaseCell, DirectionCell, EnumCell, PercentCell, PnlCell, TradesCell
from vnpy.event import Event, EventEngine
from vnpy.chart import ChartWidget, CandleItem, VolumeItem
from vnpy.trader.utility import load_json, save_json
from vnpy.trader.object import BarData, TradeData, OrderData
from vnpy.trader.database import DB_TZ
from vnpy.apps.vnpy_ctastrategy.backtesting import DailyResult
...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        self.symbol_line: QtWidgets.QLineEdit = QtWidgets.QLineEdit("SOLUSDT_SWAP.BINANCE")
        ...
        start_dt: datetime = datetime(2025, 5, 1)
        ...
        self.rate_line: QtWidgets.QLineEdit = QtWidgets.QLineEdit("0.0006")
        self.slippage_line: QtWidgets.QLineEdit = QtWidgets.QLineEdit("0.2")
        self.size_line: QtWidgets.QLineEdit = QtWidgets.QLineEdit("0.001")
        self.pricetick_line: QtWidgets.QLineEdit = QtWidgets.QLineEdit("0.1")
        self.capital_line: QtWidgets.QLineEdit = QtWidgets.QLineEdit("10000")
    ...
    def init_ui(self) -> None:
        ...
        for name, value in parameters.items():
            ...
            if type_ is int:
                ...
                edit: QtWidgets.QCheckBox = QtWidgets.QCheckBox()
                if value is True:
...
class DailyResultMonitor(BaseMonitor):
    ...
    headers: dict = {
        ...
        "trades": {"display": _("交易记录"), "cell": TradesCell, "update": False},
        "close_price": {"display": _("收盘价格"), "cell": BaseCell, "update": False},
        "long_price": {"display": _("多仓价格"), "cell": BaseCell, "update": False},
        "short_price": {"display": _("空仓价格"), "cell": BaseCell, "update": False},
        "long_pos": {"display": _("多仓数量"), "cell": BaseCell, "update": False},
        "short_pos": {"display": _("空仓数量"), "cell": BaseCell, "update": False},
        "daily_long_realized_pnl": {"display": _("多仓交易盈亏"), "cell": PnlCell, "update": False},
        "daily_short_realized_pnl": {"display": _("空仓交易盈亏"), "cell": PnlCell, "update": False},
        "daily_realized_pnl": {"display": _("交易盈亏"), "cell": PnlCell, "update": False},
        "daily_long_pnl": {"display": _("多仓持仓盈亏"), "cell": PnlCell, "update": False},
        "daily_short_pnl": {"display": _("空仓持仓盈亏"), "cell": PnlCell, "update": False},
        "daily_pnl": {"display": _("持仓盈亏"), "cell": PnlCell, "update": False},
        ...
        "long_win_rate": {"display": _("交易胜率(多)"), "cell": PercentCell, "update": False},
        "short_win_rate": {"display": _("交易胜率(空)"), "cell": PercentCell, "update": False},
    }
...
class BacktestingResultDialog(QtWidgets.QDialog):
    ...
    def on_row_clicked(self, row: int, column: int) -> None:
        """
        行点击事件，打印当行所有数据
        """

        if self.table_class == DailyResultMonitor: 
            item = self.table.item(row, column).get_data()

            print(f"交易记录")
            for trade in item.trades:
                print(f"{trade.offset.value}{trade.direction.value} {trade.price} {trade.volume}")
...
class CandleChartDialog(QtWidgets.QDialog):
    ...
    def update_trades(self, trades: list) -> None:
        ...
        for d in trade_pairs:
            ...
            open_text: pg.TextItem = pg.TextItem(f"{"L" if d["direction"] == Direction.LONG else "S"}[{volume}]", color=text_color, anchor=(0.5, 0.5))
            close_text: pg.TextItem = pg.TextItem(f"{"CL" if d["direction"] == Direction.LONG else "CS"}[{volume}]", color=text_color, anchor=(0.5, 0.5))
...
def generate_trade_pairs(trades: list) -> list:
    ...
    long_trades: list = []
    short_trades: list = []
    close_long_trades: list = []
    close_short_trades: list = []
    trade_pairs: list = []
    ...
    for trade in trades:
        ...
        if trade.direction == Direction.LONG:
            ...
            if trade.offset == Offset.OPEN:     
                ...
                opposite_direction: list = close_long_trades
            else:
                same_direction: list = close_long_trades
                opposite_direction: list = long_trades
        else:
            if trade.offset == Offset.OPEN:     
                same_direction: list = short_trades
                opposite_direction: list = close_short_trades
            else:
                same_direction: list = close_short_trades
        ...
        while trade.volume and opposite_direction:
            ...
            d: dict = {
                "open_dt": open_trade.trade_time,
                "open_price": open_trade.price,
                "close_dt": trade.trade_time,
