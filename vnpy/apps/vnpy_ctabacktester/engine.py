...
import vnpy.apps.vnpy_ctastrategy as vnpy_ctastrategy
from vnpy.apps.vnpy_ctastrategy import CtaTemplate
from vnpy.apps.vnpy_ctastrategy.backtesting import (
    BacktestingEngine,
    OptimizationSetting,
    BacktestingMode
)
...
class BacktesterEngine(BaseEngine):
    ...
    def load_strategy_class(self) -> None:
        ...
        self.load_strategy_class_from_folder(path1, "vnpy.apps.vnpy_ctastrategy.strategies")
    ...
    def run_backtesting(
        ...
        # 更新状态为数据加载中
        try:
            self.database.update_backtesting_result(result_id, status="data_loading")
            self.write_log("状态更新：数据加载中")
        except Exception as e:
            self.write_log(f"更新状态失败: {str(e)}")
        ...
        if not engine.history_data:
            ...
            # 更新状态为失败
            try:
                self.database.update_backtesting_result(result_id, status="failed")
            except Exception as e:
                self.write_log(f"更新状态失败: {str(e)}")
            self.thread = None
            return
        ...
        # 更新状态为回测中
        try:
            self.database.update_backtesting_result(result_id, status="backtesting")
            self.write_log("状态更新：回测执行中")
        except Exception as e:
            self.write_log(f"更新状态失败: {str(e)}")
        ...
        # Save trade data to database if result_id is available
        if engine.trades:
            try:
                trade_list = list(engine.trades.values())
                self.database.save_backtest_trades(result_id, trade_list)
                self.write_log(f"已保存 {len(trade_list)} 条交易记录到数据库")
            except Exception as e:
                self.write_log(f"保存交易记录到数据库失败: {str(e)}")
        
        # 更新状态为分析中
        try:
            self.database.update_backtesting_result(result_id, status="analysing")
            self.write_log("状态更新：结果分析中")
        except Exception as e:
            self.write_log(f"更新状态失败: {str(e)}")
        ...
        try:
            ...
            self.database.update_backtesting_result(
                result_id,
                total_return=statistics.get('total_return', 0),
                annual_return=statistics.get('annual_return', 0),
                max_drawdown=statistics.get('max_ddpercent', 0),
                sharpe_ratio=statistics.get('sharpe_ratio', 0),
                total_trades=statistics.get('total_trade_count', 0),
                win_rate=statistics.get('profit_days', 0) / statistics.get('total_days', 1) * 100 if statistics.get('total_days', 0) > 0 else 0,
                status="finished"
            )
            self.write_log(f"回测结果统计数据已更新到数据库，ID: {result_id}")
            self.write_log("状态更新：回测完成")
        except Exception as e:
            self.write_log(f"更新回测结果统计数据失败: {str(e)}")
            # 即使统计数据更新失败，也要更新状态为完成
            try:
                self.database.update_backtesting_result(result_id, status="failed")
            except Exception as e2:
                self.write_log(f"更新状态失败: {str(e2)}")
    ...
    def start_backtesting(
        ...
        try:
            result_id = self.database.save_backtesting_result(
                ...
                pricetick=pricetick,
                capital=capital,
                setting=setting  # 将字典转换为字符串存储
            )
            self.write_log(f"回测参数已保存到数据库，ID: {result_id}")
        except Exception as e:
            self.write_log(f"保存回测参数到数据库失败: {str(e)}")
            return None
