...
const fetchStrategyParams = async (strategyName: string) => {
  try {
    const response = await axios.get(`http://127.0.0.1:3000/backtesting/strategy_class/${strategyName}`);
...
const fetchStrategyList = async () => {
  try {
    const response = await axios.get('http://127.0.0.1:3000/backtesting/strategy_class');
