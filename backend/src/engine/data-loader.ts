...
export abstract class DataLoader {
  ...
  protected validateData(data: BarData[]): boolean {
    ...
    for (const bar of data) {
      if (!bar.timestamp || !bar.open || !bar.high || !bar.low || !bar.close || !bar.volume) {
...
//             timestamp: this.parseDateTime(fields[this.csvConfig.columns.datetime]),
//             interval: this.config.interval,
//             volume: Number.parseFloat(fields[this.csvConfig.columns.volume]),
//             open: Number.parseFloat(fields[this.csvConfig.columns.open]),
//             high: Number.parseFloat(fields[this.csvConfig.columns.high]),
//             low: Number.parseFloat(fields[this.csvConfig.columns.low]),
//             close: Number.parseFloat(fields[this.csvConfig.columns.close]),
//           };

//           // 过滤日期范围
//           if (bar.timestamp >= this.config.startDate && bar.timestamp <= this.config.endDate) {
//             bars.push(bar);
//           }
//         } catch (error) {
